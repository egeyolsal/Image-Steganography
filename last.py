...
def embed_data(img_to_embed_in, data_to_embed):
    ...
    bits_to_embed = ''.join(f"{byte:08b}" for byte in full_data_to_embed)
    ...
    if img_capacity_bits < num_bits_to_embed:
        required_pixels = ceil(num_bits_to_embed / 3.0)
        ...
        img_h, img_w = img_to_embed_in.shape[:2]
    embedded_img = img_to_embed_in.copy()
    bit_idx = 0
    for r in range(img_h):
        for c in range(img_w):
            for channel in range(3):
                if bit_idx >= num_bits_to_embed: return embedded_img
                pixel_val = embedded_img[r, c, channel] & 0xFE
                embedded_img[r, c, channel] = pixel_val | int(bits_to_embed[bit_idx])
                bit_idx += 1
    return embedded_img
...
def extract_data(stego_img):
    num_bits_for_len = 32
    all_extracted_bits = []
    img_h, img_w = stego_img.shape[:2]
    temp_bits_for_len = []
    for r in range(img_h):
        if len(temp_bits_for_len) >= num_bits_for_len: break
        for c in range(img_w):
            if len(temp_bits_for_len) >= num_bits_for_len: break
            for channel in range(3):
                if len(temp_bits_for_len) >= num_bits_for_len: break
                temp_bits_for_len.append(str(stego_img[r, c, channel] & 1))
    if len(temp_bits_for_len) < num_bits_for_len:
        raise ValueError("Veri uzunluğunu okumak için yeterli bit yok.")
    data_len_bytes = int("".join(temp_bits_for_len), 2)
    total_bits_to_extract = num_bits_for_len + data_len_bytes * 8

    for r_all in range(img_h):
        for c_all in range(img_w):
            for channel_all in range(3):
                all_extracted_bits.append(str(stego_img[r_all, c_all, channel_all] & 1))
                if len(all_extracted_bits) >= total_bits_to_extract: break
            if len(all_extracted_bits) >= total_bits_to_extract: break
        if len(all_extracted_bits) >= total_bits_to_extract: break
    
    all_bits_str = "".join(all_extracted_bits)
    if len(all_bits_str) < total_bits_to_extract:
        raise ValueError(f"Beklenenden az bit. Beklenen: {total_bits_to_extract}, Bulunan: {len(all_bits_str)}")
    data_bits_payload_str = all_bits_str[num_bits_for_len : total_bits_to_extract]
    extracted_payload_bytes = bytearray()
    for i in range(0, len(data_bits_payload_str), 8):
        byte_segment_str = data_bits_payload_str[i:i+8]
        if len(byte_segment_str) < 8: continue
        extracted_payload_bytes.append(int(byte_segment_str, 2))
    return bytes(extracted_payload_bytes)
